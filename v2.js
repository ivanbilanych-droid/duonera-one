...
const text={
cs:{login:'Přihlásit se',start:'Začít',navHow:'Jak to funguje',navSafety:'Bezpečnost',eyebrow:'SOUKROMÉ SEZNÁMENÍ PRO LIDI, KTEŘÍ CHTĚJÍ VÍC',headline:'Smysluplná seznámení',headlineAccent:'pro lidi, kteří chtějí víc',lead:'DUONERA propojuje lidi podle hodnot, životních cílů a skutečné kompatibility. Žádné nekonečné listování — jen několik kvalitních možností a kontakt po vzájemném zájmu.',cta:'Začít registraci',verified:'Schválené profily',private:'Soukromí na prvním místě',mutualOnly:'Kontakt jen po vzájemném zájmu',formEyebrow:'JEDNA REGISTRACE',formTitle:'Vše důležité na jednom místě.',formIntro:'Vyplníte základní údaje, jednu fotografii a oblast hledání. Po potvrzení e-mailu se otevře váš osobní účet.',consent:'Souhlasím se zpracováním údajů a s tím, že po schválení může být můj profil soukromě zobrazen vhodným kandidátům v DUONERA.',submit:'Dokončit registraci',formFoot:'Jedna registrace · jedna fotografie · soukromý profil · od 18 let',successEyebrow:'REGISTRACE JE HOTOVÁ',success:'Děkujeme,',successText:'Potvrďte e-mail. Potom se otevře váš osobní účet DUONERA — žádná další registrační anketa.',openAccount:'Otevřít můj účet',invalid:'Doplňte označené údaje.',photoInvalid:'Vyberte jednu fotografii JPG, PNG nebo WEBP do 12 MB.',submitting:'Dokončuji registraci…',accountExists:'Tento e-mail už má účet. Přihlaste se.',error:'Registraci se nepodařilo dokončit. Zkuste to znovu.'},
en:{login:'Sign in',start:'Start',navHow:'How it works',navSafety:'Safety',eyebrow:'PRIVATE MATCHMAKING FOR PEOPLE WHO WANT MORE',headline:'Meaningful introductions',headlineAccent:'for people who want more',lead:'DUONERA connects people through values, life goals and real compatibility. No endless swiping — just a few quality possibilities and contact after mutual interest.',cta:'Start registration',verified:'Reviewed profiles',private:'Privacy first',mutualOnly:'Contact only after mutual interest',formEyebrow:'ONE REGISTRATION',formTitle:'Everything essential in one place.',formIntro:'Add the key details, one photo and your search area. After confirming your email, your personal account opens.',consent:'I agree to data processing and to my approved profile being shown privately to suitable DUONERA candidates.',submit:'Finish registration',formFoot:'One registration · one photo · private profile · 18+',successEyebrow:'REGISTRATION COMPLETE',success:'Thank you,',successText:'Confirm your email. Your personal DUONERA account will then open — there is no second registration form.',openAccount:'Open my account',invalid:'Complete the highlighted details.',photoInvalid:'Choose one JPG, PNG or WEBP photo up to 12 MB.',submitting:'Finishing registration…',accountExists:'This email already has an account. Sign in.',error:'Registration could not be completed. Please try again.'}
};
let active='cs';
...
form.addEventListener('submit',async event=>{
 ...
 const profile={id,first_name:String(data.get('first_name')).trim(),age:Number(data.get('age')),gender:gender(data.get('gender')),looking_for:seeking(data.get('looking_for')),city:String(data.get('city')).trim(),country:countryDefaults[country]?.country||locale[active]?.country||'Česko',languages:[locale[active]?.language||'Čeština'],email,preferred_distance_km:Number(data.get('distance')||50),consent_privacy:true,consent_discovery:true,landing_language:active,source:'duonera.cz/registration',registration_stage:'complete'};
 const button=form.querySelector('.submit'),label=button.querySelector('span'),original=label.textContent;
 try{
   button.disabled=true;label.textContent=text[active].submitting;
   await savePendingRegistrationPhoto(email,preparedPhoto);
   const redirect=`${location.origin}/v2-app.html?registration=complete`;
   const registration=await registerMember(email,String(data.get('password')||''),redirect,profile);
   try{await insertRow('duonera_leads',{id,gender:profile.gender,looking_for:profile.looking_for,age:profile.age,city:profile.city,email:profile.email,consent_privacy:true,source:profile.source},20000)}catch(error){console.warn('Lead save skipped',error)}
   ...
   document.querySelector('[data-account-after-registration]').href=`v2-app.html?mode=login&email=${encodeURIComponent(email)}`;
   dialog.close();document.querySelector('.success').hidden=false;
   if(registration?.user?.id&&typeof gtag==='function')gtag('event','sign_up',{method:'email_password'});
 }catch(error){console.error(error);showToast(error?.code==='account_exists'?text[active].accountExists:text[active].error)}finally{button.disabled=false;label.textContent=original}
});
